...
const MemberList = ({ room, presences, setShowMembers }: { room: Room, presences: Record<string, IStatusResponse>, setShowMembers: (_: boolean) => void }) => {
  const [memberListWidth, setMemberListWidth] = useState(300);

  const sortedMembers = room
            .getMembers().sort(sortMembers);
    ...
    >
      ...
      <div className="basis-1/4">
        <ul className="flex flex-col">
          ...
          <p>members</p>
          {sortedMembers.filter(m => m.powerLevel < 100).map(m => (
              <MemberChip presence={presences[m.userId]} member={m} />
          ))}
        </ul>
      </div>
...
const MemberChip = ({ member, presence }: { member: RoomMember, presence?: IStatusResponse }) => {
return (

              <li className={`border-2 ${presence && presence.presence === "offline" ? "opacity-50" : null}`}>
                <div className="flex gap-4 items-center">
                  ...
                  <div>
                    <p>{member.name} {member.powerLevel}</p>
                  </div>
                </div>
              </li>
)
