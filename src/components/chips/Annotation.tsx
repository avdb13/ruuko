...
const Annotation = ({ annotation, annotators }: { annotation: MatrixEvent, annotators: RoomMember[] }) => {
  ...
  return (
    ...
    <div className="inline-flex items-center bg-gray-50 hover:bg-gray-100 transition-all text-gray-600 px-2 py-1 ring-4 ring-inset ring-gray-500/10 rounded-md">
      <button onClick={handleClick}>{key?.startsWith("mxc") ? <img src={src} className="h-5" alt={key} /> : key}</button>
    </div>
    <div className="hidden bg-black text-white font-bold p-4 rounded-md group-hover:block">
      <p>{anchorContent}</p>
    </div>
    </div>
  )
